...
** Static list from static code
** Add some interactions: when you click on a list item, it's toggled grey / normal
* Handle state mutation through ~reframe~
* Ask Thibault for sketches and his need
:DOD: Thibault and I have agreed on a clear statement on what is the goal
* Refactor code from boilerplate state to maintainable state
...
* TBD: sketch upon Thibault idea and need
